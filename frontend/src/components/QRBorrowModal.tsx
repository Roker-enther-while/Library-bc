...
const QRBorrowModal: React.FC<QRBorrowModalProps> = ({ work, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-modalIn">
        <div className="bg-gradient-to-r from-jade to-jade-light text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/80 font-sans">Hệ thống QR mượn sách</p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 bg-parchment border-b border-parchment-dark flex items-center gap-3">
          <div
          ...
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-ink-dark line-clamp-1">{work.title}</h3>
            <p className="text-xs text-ink-light/60 font-sans">{work.author?.name} · {work.publicationYear}</p>
          </div>
        </div>

        {step === 'form' && (
          <form onSubmit={handleBorrow} className="p-6 space-y-4">
            <p className="text-sm text-ink-light/70 font-sans">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-parchment-dark bg-parchment text-sm font-sans focus:outline-none focus:ring-2 focus:ring-jade/30 focus:border-jade"
                required
              />
            </div>
            <div>
            ...
            <div className="bg-gold/10 rounded-xl p-3 flex items-start gap-2">
              <Clock size={14} className="text-gold-dark mt-0.5 flex-shrink-0" />
              <p className="text-xs text-gold-dark font-sans">
                Thời hạn mượn: <strong>14 ngày</strong>. Vui lòng trả sách đúng hạn.
              </p>
            </div>
            <button
              type="submit"
              className="w-full py-3 bg-gradient-to-r from-jade to-jade-light text-white font-sans font-bold text-sm rounded-xl hover:shadow-lg transition-all flex items-center justify-center gap-2"
            >
              <QrCode size={18} />
              Tạo mã QR mượn sách
            </button>
        ...
        {step === 'qr' && record && (
          <div className="p-6 space-y-4">
            <div className="flex justify-center">
              <div className="qr-container bg-white p-4 rounded-2xl border-2 border-jade/20 shadow-inner inline-block">
            ...
            <div className="bg-parchment rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-light/60 font-sans">Mã mượn</span>
                <span className="text-xs font-sans font-bold text-jade">{record.id}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-light/60 font-sans">Người mượn</span>
                <span className="text-xs font-sans font-semibold text-ink">{record.borrowerName}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-light/60 font-sans">SDT</span>
                <span className="text-xs font-sans text-ink">{record.borrowerPhone}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-light/60 font-sans">Ngày mượn</span>
                <span className="text-xs font-sans text-ink">{record.borrowDate}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-ink-light/60 font-sans">Hạn trả</span>
                <span className="text-xs font-sans font-semibold text-vermillion">{record.dueDate}</span>
              </div>
            </div>
            <div className="bg-jade/10 rounded-xl p-3 flex items-start gap-2">
              <Smartphone size={14} className="text-jade mt-0.5 flex-shrink-0" />
              <p className="text-xs text-jade-dark font-sans">
        ...
        {step === 'success' && record && (
          <div className="p-8 text-center space-y-4">
            ...
            <div>
              <h4 className="font-display text-xl font-bold text-ink mb-2">Mượn sách thành công!</h4>
              <p className="text-sm text-ink-light/60 font-sans">
                Mã mượn: <span className="font-bold text-jade">{record.id}</span>
              </p>
            </div>
            <div className="bg-parchment rounded-xl p-4">
              ...
              <p className="text-xs font-sans text-ink-light/60 mt-1">
                Hạn trả: <strong className="text-vermillion">{record.dueDate}</strong>
              </p>
            </div>
            ...
            <button
              onClick={onClose}
              className="w-full py-3 bg-ink text-white font-sans font-bold text-sm rounded-xl hover:bg-ink-light transition-colors"
            >
              Đóng
            </button>
          </div>
