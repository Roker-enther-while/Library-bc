...
const BookCard: React.FC<BookCardProps> = ({ work, onRead, onReserve, variant = 'default', index = 0 }) => {
    ...
    return (
      <div className={`group relative overflow-hidden rounded-2xl book-card-shadow bg-white dark:bg-dark-card w-full animate-fadeInUp stagger-${Math.min(index + 1, 8)}`}>
        <div className="flex flex-col sm:flex-row">
          <button
            onClick={() => onRead(work)}
            className="book-3d sm:w-48 h-48 sm:h-auto flex-shrink-0 flex items-center justify-center relative overflow-hidden text-left"
            style={{ backgroundColor: work.coverColor }}
          >
            <div className="book-3d-inner w-full h-full flex items-center justify-center">
              <div className="book-cover-3d w-full h-full flex items-center justify-center">
    ...
    return (
      ...
      >
        <div
          ...
          style={{ backgroundColor: work.coverColor + '20' }}
        >
          📘
        </div>
  ...
  return (
    <div
      className={`group book-3d animate-fadeInUp stagger-${Math.min(index + 1, 8)}`}
      style={{ opacity: 0 }}
    >
      <div className="book-3d-inner rounded-2xl overflow-hidden book-card-shadow bg-white dark:bg-dark-card">
        <button
          onClick={() => onRead(work)}
          className="book-cover-3d relative w-full aspect-[3/4] flex items-center justify-center overflow-hidden"
          style={{ backgroundColor: work.coverColor }}
        >
        ...
        <div className="p-4">
          ...
          {work.shelfLocation && (
            <div className="flex items-center gap-1 mt-1 text-[10px] text-ink/50 dark:text-gray-500 font-sans italic">
