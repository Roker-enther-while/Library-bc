...
import { Author, LiteraryWork } from '../types';
import { getAuthors, getBooks } from '../services/api';
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filtered.map((author, i) => {
                const authorWorks = works_dynamic.filter(w => (w.author?.id || w.authorId) === author.id);
                return (
                  <button
                    key={author.id}
                    onClick={() => navigate(`/tac-gia/${author.id}`)}
                    className={`group bg-white rounded-2xl book-card-shadow overflow-hidden text-left animate-fadeInUp stagger-${Math.min(i + 1, 8)}`}
                  >
                    <div className="flex">
                      <div className="w-28 sm:w-36 bg-gradient-to-br from-parchment to-parchment-dark flex items-center justify-center flex-shrink-0 overflow-hidden">
                      ...
                      <div className="flex-1 p-5 sm:p-6">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3 className="font-display text-lg sm:text-xl font-bold text-ink group-hover:text-vermillion transition-colors">
                              {author.name}
                            </h3>
                            <div className="flex items-center gap-3 mt-1">
                              <span className="flex items-center gap-1 text-xs text-ink-light/50 font-sans">
                                <Calendar size={12} />
                        ...
                        <p className="text-xs text-ink-light/60 font-sans leading-relaxed line-clamp-2 mt-2 mb-3">
                          {author.bio}
                        </p>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="font-display text-xl font-bold text-ink mb-2">Không tìm thấy tác giả</h3>
                <p className="text-sm text-ink-light/60 font-sans">Thử thay đổi từ khóa tìm kiếm</p>
              </div>
            )}
          </>
        )}
